 ...
 */
export const findOrCreateCustomer = async (lineUserId, lineDisplayName) => {
 ...
 * 購入履歴をNotionに記録する
 */
export const recordPurchase = async (customerPageId, itemName, memo) => {
 ...
 */
export const getHistory = async (customerPageId) => {
...
export const getProductList = async () => {
    ...
    return response.results.map((page) => page.properties['商品名']?.title?.[0]?.plain_text?.trim() || '').filter((name) => !!name);
};
